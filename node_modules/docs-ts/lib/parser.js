...
var ordModule = pipeable_1.pipe(Ord_1.ordString, Ord_1.contramap(function (module) { return module.path.join('/').toLowerCase(); }));
var sortModules = A.sort(ordModule);
var monoidFailure = A.getMonoid();
exports.monadParser = E.getValidation(monoidFailure);
var traverse = A.array.traverse(exports.monadParser);
function run(files) {
    return pipeable_1.pipe(traverse(files, function (file) { return parse(file.path.split(path.sep), file.content); }), E.map(function (modules) { return sortModules(modules.filter(function (module) { return !module.deprecated; })); }));
}
exports.run = run;
...
function getDescription(annotation) {
    return pipeable_1.pipe(O.fromNullable(annotation.description), O.filter(function (s) { return s !== ''; }));
}
function getFile(annotation) {
    return pipeable_1.pipe(O.fromNullable(annotation.tags.filter(function (tag) { return tag.title === 'file'; })[0]), O.mapNullable(function (tag) { return tag.description; }));
}
function getSince(annotation) {
    return pipeable_1.pipe(O.fromNullable(annotation.tags.filter(function (tag) { return tag.title === 'since'; })[0]), O.mapNullable(function (tag) { return tag.description; }));
}
function isDeprecated(annotation) {
    return pipeable_1.pipe(O.fromNullable(annotation.tags.filter(function (tag) { return tag.title === 'deprecated'; })[0]), O.isSome);
}
function isInternal(annotation) {
    return pipeable_1.pipe(O.fromNullable(annotation.tags.filter(function (tag) { return tag.title === 'internal'; })[0]), O.isSome);
}
function getExamples(annotation) {
    return annotation.tags
        .filter(function (tag) { return tag.title === 'example'; })
        .map(function (tag) {
        return pipeable_1.pipe(O.fromNullable(tag.description), O.getOrElse(function () { return ''; }));
    });
}
...
function getInterfaces(sourceFile) {
    ...
    return pipeable_1.pipe(traverse(exportedInterfaceDeclarations, parseInterfaceDeclaration), E.map(function (interfaces) { return interfaces.sort(byName.compare); }));
}
...
function getFunctionVariableDeclarationSignature(vd) {
    var text = vd.getText();
    var end = pipeable_1.pipe(indexOf(text, ' => {'), O.alt(function () { return lastIndexOf(text, ' =>'); }));
    return "export const " + text.substring(0, pipeable_1.pipe(end, O.getOrElse(function () { return text.length; }))) + " => ...";
}
...
var byName = pipeable_1.pipe(Ord_1.ordString, Ord_1.contramap(function (x) { return x.name; }));
function getFunctions(moduleName, sourceFile) {
    ...
    var functionDeclarations = traverse(exportedFunctionDeclarations, function (fd) { return parseFunctionDeclaration(moduleName, fd); });
    ...
    var variableDeclarations = traverse(exportedVariableDeclarations, parseFunctionVariableDeclaration);
    var monoidFunc = E.getValidationMonoid(monoidFailure, A.getMonoid());
    return pipeable_1.pipe(monoidFunc.concat(functionDeclarations, variableDeclarations), E.map(function (funcs) { return funcs.sort(byName.compare); }));
}
...
function getTypeAliases(sourceFile) {
    ...
    return pipeable_1.pipe(traverse(exportedTypeAliasDeclarations, function (ta) { return parseTypeAliasDeclaration(ta); }), E.map(function (typeAliases) { return typeAliases.sort(byName.compare); }));
}
...
function getConstants(sourceFile) {
    ...
    return pipeable_1.pipe(traverse(exportedVariableDeclarations, parseConstantVariableDeclaration), E.map(function (constants) { return constants.sort(byName.compare); }));
}
...
function getExports(sourceFile) {
    ...
    return pipeable_1.pipe(traverse(exportDeclarations, parseExportDeclaration), E.map(function (exports) { return exports.sort(byName.compare); }));
}
...
function parseClass(moduleName, c) {
    ...
    if (name === undefined) {
        ...
        return pipeable_1.pipe(Apply_1.sequenceS(E.either)({
            methods: traverse(c.getInstanceMethods(), parseMethod),
            staticMethods: traverse(c.getStaticMethods(), parseMethod)
        }), E.map(function (_a) {
            var methods = _a.methods, staticMethods = _a.staticMethods;
...
function getClasses(moduleName, sourceFile) {
    ...
    return pipeable_1.pipe(traverse(exportedClasses, function (cd) { return parseClass(moduleName, cd); }), E.map(function (classes) { return classes.sort(byName.compare); }));
}
...
function parse(path, source) {
    ...
    return pipeable_1.pipe(Apply_1.sequenceS(exports.monadParser)({
