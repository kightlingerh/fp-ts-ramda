...
function printStaticMethod(f) {
    var s = h2(handleDeprecated(f.name, f.deprecated) + ' (static method)');
    s += printDescription(f.description);
    s += printSignatures(f.signatures);
    s += printExamples(f.examples);
    s += printSince(f.since);
...
function printDescription(description) {
    return pipeable_1.pipe(description, O.fold(function () { return ''; }, function (s) { return CRLF + s; }));
}
function printModuleDescription(description) {
    return pipeable_1.pipe(description, O.fold(function () { return ''; }, function (s) { return CRLF + h1('Overview') + CRLF + s + CRLF; }));
}
...
function printSince(since) {
    return pipeable_1.pipe(since, O.fold(function () { return ''; }, function (s) { return CRLF + ("Added in v" + s); }));
}
